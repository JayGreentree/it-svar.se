...
class Ep_entry_draft extends CI_Model {
	...
	// Use the &$var_name syntax to set up a reference to the original variable rather than copying it
	...
	function _set_edit_date_as_unix(&$data)
	{
		$data['edit_date'] = time();
	}
	...
	function create($attr_values, $drop_non_existing_columns=TRUE)
	{
		...
		$this->_set_edit_date_as_unix($attr_values);
	...
	{		
		...
		$rs = $this->db->query("SELECT $fields FROM exp_ep_entry_drafts WHERE entry_id IN ($entry_ids_str)");

		foreach ($rs->result_array() as $row)
		{
			$bwf_draft_data[] = array('entry_id' => $row['entry_id'], 'status' => $row['status']);
		}
