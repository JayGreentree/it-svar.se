...
class Ep_draft_utils
{
	...
	*
	*/
	function load_draft($draft, $data, &$is_draft)
	{
		// Set the default return to be the entry's data
		$out = $data;

		// unserialise the draft data element from our record set
		$draft_data = unserialize($draft->draft_data); 

		// Loop through the draft_data array making sure we don't have a revision_post element
		foreach($draft_data as $k => $v)
		{
			if ($k != 'revision_post') $out[$k]=$v;
		}
		
		// Double check that we have a status for our draft
		if(!isset($out['status'])) $out['status'] = $draft->status;
